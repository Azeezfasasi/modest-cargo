...
const TAGS = ['modest cargo', 'shipping', 'customer', 'USA', 'Nigeria'];
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      for (const file of files) {
        try {
          const result = await uploadImageToCloudinary(file, 'modest/gallery');
          
          // Handle both string URL and object responses
          const imageUrl = typeof result === 'string' ? result : (result?.url || result?.secure_url);
          const publicId = typeof result === 'object' ? (result?.publicId || result?.public_id) : '';
          
          if (!imageUrl) {
            throw new Error(`Failed to get URL for ${file.name}. Response: ${JSON.stringify(result)}`);
          }
