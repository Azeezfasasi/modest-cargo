		...
		if (search.trim()) {
			filtered = filtered.filter((shipment) => {
				...
				return (
					...
					(shipment.trackingNumber || shipment.trackingNumber || '').toLowerCase().includes(searchStr)
				)
			})
		}
									...
									{currentShipments.map((shipment, idx) => (
										<tr key={shipment._id || shipment.id || idx} className="hover:bg-gray-50 transition">
											<td className="px-6 py-4">
												<span className="inline-flex px-3 py-1 rounded-full text-xs font-medium bg-green-50 text-red-700">
