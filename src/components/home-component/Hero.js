  ...
  if (loading) {
    return (
      <section className="w-full">
        <div className="h-[420px] md:h-[540px] flex items-center justify-center bg-red-50">
            ...
            {slides.map((s, i) => (
              <div key={s._id} className="min-w-full flex-none" style={{ flex: '0 0 100%' }}>
                <div className="h-[420px] md:h-[540px] flex items-center">
                  <div className="w-full h-full p-8 md:p-12 flex items-center justify-between gap-6 bg-gradient-to-br from-blue-200 to-blue-100">
                    <div className="flex-1 max-w-full md:max-w-2xl">
                      ...
                      <div className="flex gap-3 flex-wrap">
                        <Link href={s.ctaHref} className="inline-block px-2 md:px-5 py-3 bg-red-700 text-white rounded-md font-medium hover:bg-red-800 transition-colors">{s.ctaLabel}</Link>
                        <Link href="/about-us" className="inline-block px-2 md:px-5 py-3 border border-red-700 rounded-md text-red-700 hover:bg-white/50 transition-colors">Learn more</Link>
                      </div>
                    </div>
          ...
          {slides.length > 1 && (
            <>
              <button
                ...
                className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/30 md:bg-white/80 hover:bg-white p-0.5 md:p-2 rounded-full shadow-md text-red-700 cursor-pointer transition-all"
              >
                <ArrowBigLeftDash />
              </button>
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/30 md:bg-white/80 hover:bg-white p-0.5 md:p-2 rounded-full shadow-md text-red-700 cursor-pointer transition-all"
              >
                <ArrowBigRightDash />
              </button>          
              ...
              <div className="absolute left-1/2 -translate-x-1/2 bottom-6 flex gap-2">
                {slides.map((_, i) => (
                  <button
                    ...
                    onClick={() => setIndex(i)}
                    className={`w-3 h-3 rounded-full transition-all ${i === index ? 'bg-red-700' : 'bg-white/70 border border-gray-200'}`}
                  />
                ))}
              </div>
            </>
