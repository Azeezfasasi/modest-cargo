...
import { Anchor, Boxes, Clock, Container, Gauge, Globe, Loader, MapPin, Navigation, Package, Plane, Route, Shield, Ship, Truck, Weight, Wind, Zap, Handshake, BookText } from 'lucide-react'
...
function ServiceIcon({ name }) {
  ...
  switch ((name || '').toLowerCase()) {
    ...
    // Legacy support for old service names
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-gradient-to-r from-red-600 to-red-700 text-white p-6 flex items-start justify-between">
            ...
            {services.map((service) => (
              ...
              >
                <div className="flex flex-col items-start gap-4 h-full">
                  <div className={`bg-gradient-to-br ${service.color || 'from-red-100 to-red-200'} p-4 rounded-lg text-white w-12 h-12 flex items-center justify-center shrink-0`}>
                    <ServiceIcon name={service.icon || service.title} />
                  </div>
                  <div className="grow">
                  ...
                  <button className="text-red-600 font-semibold text-sm hover:text-red-700 mt-4 flex items-center gap-1">
                    Learn More
